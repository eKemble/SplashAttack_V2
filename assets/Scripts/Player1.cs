...
public class Player1: Player {
    ...
    void FixedUpdate()
    {
        float controllerHorizontal = Input.GetAxis("Controller1Horizontal") * 50;
        ...
        if (Input.GetKeyDown("joystick 1 button 1") && canJump == true && !stall)
        {
            //NORMAL JUMP
            if (!running)
            {
        ...
        if (Input.GetKeyDown("joystick 1 button 3") && canJump == true && bursts > 0)//BIG JUMP
        {
        ...
        if (Input.GetKeyDown("joystick 1 button 0") /*CONTROLLER*/&& bursts > 0) //Burst Left
        {
        ...
        if (Input.GetKeyDown("joystick 1 button 2") /*CONTROLLER*/ && bursts > 0)//Burst Right
        {
            myMovement.x = jumpingForce;
            myRigidbody.AddForce(myMovement);
            myMovement = Vector3.zero;
        ...
        if (Input.GetButtonDown("Controller1Fire2"))
        {
